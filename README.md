...
A little game to help you learn geography. 
...
The core of the game is a geography -- a set of polygons that are part of a place -- such as Massachusetts towns or US states.

Each geography needs two files, in a namespaced subdirectory:

- `[place]/[place].topo.json`: A [topojson](https://github.com/topojson/topojson) file containing boundaries within this place
- `[place]/[place].metadata.json`: A metadata file telling the app about the topojson file.

So, for Massachusetts towns, the full filenames should be:

- `massachusetts-towns/massachusetts-towns.topo.json`
- `massachusetts-towns/massachusetts-towns.metadata.json`

A `Makefile` included in the project should describe how to download and transform each geography. Running `make all` should construct every geography we have.

A separate `make` recipe should ensure `[place]/[place].metadata.json` exists, but anything beyond that is up to the user adding the geography.

*Note on files*: When downloading, please use the ignored temporary directories `tmp/zip`, `tmp/shp` and `tmp/json` to ensure our ETL detritus doesn't accidentally end up in the repo.
